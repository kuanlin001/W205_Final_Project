...
class ParseMessage (Bolt):
	...
	col_pid = raw_fields[0]
        ...
        if len(raw_fields) == 10: #1 to 3 measures for 1 to 3 out tuples
            ...
            for field in raw_fiels[7:]: #up to 3 of them
                col_type = field.split(':')[0].strip()
                col_value = field.split(':')[1].split(' ')[1].strip()
                if col_value != 'N/A':
                    col_unit = field.split(':')[1].split(' ')[2].strip()
                    out_tuples = out_tuples.append({'pid':col_pid, 'type':col_type, 'value': col_value, 'unit':col_unit, 'date': col_date, 'time': col_time})

        elif len(raw_fields) == 9: #only one measure for 1 out tuple
                col_type = (raw_fields[7].strip() + '_' + raw_fileds[8].split(':')[0].strip()).replace('.','_')
                col_value_unit = raw_fileds[8].split(':')[1].strip().split(' ')
                if len(col_value_unit) == 2:
                   col_value, col_unit = col_value_unit[0], cl_value_unit[1]
                   out_tuples = out_tuples.append({'pid':col_pid, 'type':col_type, 'value': col_value, 'unit':col_unit, 'date': col_date, 'time': col_time}) 


	# Emit all out_tuples and save them to the mongodb
	for out_tupple in out_tuple_fields:
